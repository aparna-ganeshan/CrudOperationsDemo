...
const ApiCalling = () => {
  ...
  const [showTable, setShowTable] = useState(true); // State to control the visibility of the table
  ...
  const handleAddItem = () => {
    ...
    setShowInputs(false); // Hide input fields after adding a product
    setShowTable(true); // Show the table after adding a product
      ...
      <div className="navContainer">
        <div className="addingButton">
          <button className="buttonData" type="button" onClick={toggleInputs}>
            {showInputs ? 'Hide' : 'Show'} Add New Item Form
          </button>
        </div>
      </div>
